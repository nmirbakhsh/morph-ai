...
export default function App() {
  ...
  const goDirection = useCallback(
    async (direction: Direction, intent?: IntentSignpost) => {
      if (navLockRef.current || archiveOpen) return;
      ...
      es.addEventListener("log", (e) => appendLog((e as MessageEvent).data));
  ...
  useEffect(() => {
    ...
    for (const intent of currentNode.adjacent_intents.intents) {
      if (intent.is_back || intent.target_node_id) continue;
      ...
      if (state.nodesByCoord[k]) continue;
      if (state.prefetching.has(k)) continue;
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      ...
      if (Math.abs(acc.x) > Math.abs(acc.y)) {
        if (acc.x > WHEEL_THRESHOLD) dir = "right";
        else if (acc.x < -WHEEL_THRESHOLD) dir = "left";
      } else {
        if (acc.y > WHEEL_THRESHOLD) dir = "down";
        else if (acc.y < -WHEEL_THRESHOLD) dir = "up";
      }
      if (dir) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (archiveOpen) {
        if (e.key === "Escape") useStore.getState().toggleArchive();
        return;
      }
      ...
      switch (e.key) {
        ...
        case "ArrowDown":  goDirection("down");  break;
        case "ArrowUp":    goDirection("up");    break;
        case "a":          useStore.getState().toggleArchive(); break;
        case "/":          e.preventDefault(); useStore.getState().toggleChat(); break;
      }
    };
  ...
  useEffect(() => {
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > Math.abs(dy)) {
        if (dx > TOUCH_THRESHOLD) dir = "left";
        else if (dx < -TOUCH_THRESHOLD) dir = "right";
      } else {
        if (dy > TOUCH_THRESHOLD) dir = "up";
        else if (dy < -TOUCH_THRESHOLD) dir = "down";
      }
      if (dir) goDirection(dir);
    };
  ...
  return (
    <>
      ...
      <TopBar panelLabel={currentNode?.title ?? "Loading…"} />

      {currentNode && !pending && (
        <IntentSignposts
          intents={currentNode.adjacent_intents.intents}
          onPick={(it) => goDirection(it.direction, it)}
        />
      )}
      ...
      <div className="coord-readout">
        XY {currentCoord[0]}, {currentCoord[1]}
